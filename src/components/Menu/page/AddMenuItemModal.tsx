...
export default function AddMenuItemModal({
  ...
  const validate = (): boolean => {
    ...
    if (!form.category) {
      newErrors.category = t("pleaseSelectACategory");
    }

    if (form.price === "" || Number(form.price) < 0) {
      newErrors.price = t("pleaseEnterAValidPrice");
    }
        ...
        <div className="px-5 sm:px-8 pt-6 pb-4 border-b border-blue-100 flex-shrink-0">
          <div className="flex items-start justify-between">
            <div className="min-w-0 pr-3">
              <h2 className="text-lg sm:text-xl font-bold text-gray-900">{t("addMenuItem")}</h2>
              <p className="text-sm text-gray-500 mt-0.5">
                {t("fillInTheDetailsForTheNewItem")}
              </p>
            </div>
            <button
              onClick={handleClose}
