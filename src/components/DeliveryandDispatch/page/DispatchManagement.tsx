...
function AddDriverModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState<DriverFormData>({
    name: "", phone: "", email: "", vehicleType: "", vehiclePlate: "", assignedAreas: "",
  });
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          {errCount > 0 && Object.keys(touched).length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-center gap-2">
              ...
              <p className="text-sm text-red-700 font-medium">
                {errCount} {t("fieldsNeedAttention")}
              </p>
            </div>
          )}
          {apiError && (
              ...
              {VEHICLE_TYPES.map((vt) => (
                <button key={vt.value} type="button"
                  ...
                  className={`py-2.5 rounded-xl border-2 text-sm font-semibold transition ${
                    form.vehicleType === vt.value ? "border-blue-500 bg-blue-50 text-blue-700"
                      : errors.vehicleType ? "border-red-300 bg-white text-gray-600"
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2">
          <button onClick={onClose} disabled={loading} className="px-5 py-2 rounded-xl border border-gray-200 text-gray-600 text-sm font-semibold hover:bg-gray-50 transition disabled:opacity-50">
            {t("cancel")}
          </button>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {driver.todayStats && (
            <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-sm text-blue-700 flex items-center gap-2">
              <span>📅</span> {t("todayStats")}: <strong>{driver.todayStats.assigned ?? 0}</strong> {t("assigned")} ·{" "}
              <strong>{driver.todayStats.delivered ?? 0}</strong> {t("delivered")}
            </div>
          )}
        </div>
...
interface AssignDriverErrors {
  driverId?: string; deliveryFee?: string; commission?: string;
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input type="text" placeholder="e.g. Gehan street villa 7, Mansoura" value={address}
          ...
          <div>
            ...
            {availableDrivers.length === 0 ? (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700 flex items-center gap-2">
                <span>⚠️</span> {t("noAvailableDrivers")}
              </div>
            ) : (
              <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
                {availableDrivers.map((d) => (
                  <button key={d.id} type="button"
                    onClick={() => { setDriverId(d.id ?? ""); setErrors((p) => ({ ...p, driverId: undefined })); }}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 text-left transition ${
                      driverId === d.id ? "border-blue-500 bg-blue-50"
                        : errors.driverId ? "border-red-300 bg-white hover:border-blue-300"
              ...
              <div key={key}>
                ...
                <div className="relative">
                  ...
                  <input type="number" min="0" step="0.5" value={val}
                    onChange={(e) => { set(e.target.value); setErrors((p) => ({ ...p, [errKey]: undefined })); }}
                    className={`w-full bg-gray-50 border rounded-xl pl-7 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:bg-white transition ${err ? "border-red-400 focus:ring-red-300" : "border-gray-200 focus:ring-blue-500"}`}
                  />
                </div>
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input value={phone} onChange={(e) => { setPhone(e.target.value); setPhoneErr(""); }}
                className={`w-full bg-gray-50 border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition ${phoneErr ? "border-red-400" : "border-gray-200"}`}
              />
          ...
          <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-2">
            ...
            <div className="flex justify-between text-sm font-bold text-gray-900 pt-2 border-t border-gray-200">
              <span>{t("totalLabel")}</span><span>${total.toFixed(2)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2">
          ...
          <button
            onClick={() => {
              ...
              if (!PHONE_RE.test(phone.trim())) { setPhoneErr("Invalid phone number"); return; }
              setPhoneErr(""); onClose();
            }}
...
export default function DispatchManagement() {
  ...
  const { data: dispatchData, isLoading: dispatchLoading, isError: dispatchError, error: dispatchRawError, refetch: refetchDispatches } = useDispatches({ sort: "-assignedAt", limit: 10, page });
  ...
  const handleDeleteDriver = useCallback(async (driverId: string) => {
    ...
    try { await deleteDriverFn(driverId); invalidateQuery("drivers"); }
      ...
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                {dispatchError && !dispatchLoading && (
                  <tr><td colSpan={9} className="text-center py-12">
                    <p className="text-red-500 text-sm mb-2">
                      {t("failedToLoad")}
                      {(dispatchRawError as any)?.response?.data?.message ? ` — ${(dispatchRawError as any).response.data.message}` : ""}
                    </p>
                    ...
                    <tr key={id} className="border-b border-gray-50 hover:bg-gray-50/70 transition">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 flex-wrap">
                          ...
                          {status === "assigned" && (
                            <button onClick={() => handleStatusChange(id, "picked-up")} disabled={isUpdating} className="px-2 py-1 rounded-lg bg-purple-100 text-purple-700 text-xs font-semibold hover:bg-purple-200 transition disabled:opacity-50 whitespace-nowrap">
                              {isUpdating ? "..." : `📦 ${t("pickUp")}`}
                            </button>
                          )}
                          {status === "picked-up" && (
                            <button onClick={() => handleStatusChange(id, "out-for-delivery")} disabled={isUpdating} className="px-2 py-1 rounded-lg bg-yellow-100 text-yellow-700 text-xs font-semibold hover:bg-yellow-200 transition disabled:opacity-50 whitespace-nowrap">
                              {isUpdating ? "..." : `🛵 ${t("out")}`}
                            </button>
                          )}
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
            <span className="text-xs text-gray-500">{t("showing")} <strong>{filtered.length}</strong> {t("of")} <strong>{totalDocs}</strong></span>
            <div className="flex items-center gap-1">
              <button disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="w-7 h-7 rounded-lg text-xs text-gray-600 border border-gray-200 hover:bg-gray-50 disabled:opacity-40">‹</button>
              {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map((p) => (
                <button key={p} onClick={() => setPage(p)} className={`w-7 h-7 rounded-lg text-xs font-semibold ${page === p ? "bg-blue-600 text-white" : "hover:bg-gray-100 text-gray-600"}`}>{p}</button>
              ))}
              <button disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="w-7 h-7 rounded-lg text-xs text-gray-600 border border-gray-200 hover:bg-gray-50 disabled:opacity-40">›</button>
            </div>
          </div>
        </div>
                  ...
                  {drivers.map((driver) => {
                    ...
                    return (
                      <tr key={dId} className="border-b border-gray-50 hover:bg-gray-50/70 transition">
                        <td className="px-4 py-3"><p className="font-medium text-gray-800 text-sm">{driver.name ?? "—"}</p><p className="text-xs text-gray-400">{driver.phone ?? ""}</p></td>
                        <td className="px-4 py-3"><p className="text-sm text-gray-700 capitalize">{driver.vehicleType ?? "—"}</p><p className="text-xs text-gray-400">{driver.vehiclePlate ?? ""}</p></td>
                        <td className="px-4 py-3 max-w-[130px]"><p className="text-xs text-gray-500 truncate" title={(driver.assignedAreas ?? []).join(", ")}>{(driver.assignedAreas ?? []).join(", ") || "—"}</p></td>
                        <td className="px-4 py-3"><span className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${driverStatusColors[statusKey] ?? "bg-gray-100 text-gray-500"}`}>{statusKey}</span></td>
                        <td className="px-4 py-3"><p className="text-sm text-gray-700 font-medium">{(driver.totalDeliveries ?? 0).toLocaleString()}</p><p className="text-xs text-gray-400">⭐ {driver.rating ?? 0} · {t("today")}: {driver.todayStats?.delivered ?? 0}✓</p></td>
