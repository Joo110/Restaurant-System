...
// ── Assign Driver Modal ────────────────────────────────────────────────────────
interface DispatchAssignDriverErrors {
  driverId?: string;
  deliveryFee?: string;
  commission?: string;
  address?: string;
...
function validateDispatchAssign(
  driverId: string,
  deliveryFee: string,
  commission: string,
  address: string,
  phone: string
): DispatchAssignDriverErrors {
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full bg-gray-50 border rounded-xl pl-7 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:bg-white transition ${
                      err ? "border-red-400 focus:ring-red-300" : "border-gray-200 focus:ring-blue-500"
                    }`}
                  />
                </div>
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500">
              {t("order")} #{(dispatch as any).orderNumber ?? dispatch._id?.slice(-6)}
            </p>
          </div>
          <button
            onClick={onClose}
...
export default function DispatchManagement() {
  ...
  const pagination = dispatchData?.paginationResult ??
    (dispatchData as any)?.pagination ??
    (dispatchData as any)?.meta ?? {};

  const totalDocs: number  = pagination?.totalDocs ?? pagination?.total ?? (dispatchData as any)?.results ?? 0;
  const totalPages: number = Math.max(1, pagination?.totalPages ?? pagination?.pages ??
  ...
  // page window: up to 5 buttons around current
  const pageWindow = (() => {
    const half  = 2;
  ...
  // search is now sent to server — no client-side filtering needed
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {[t("orderId"), t("branch"), t("area"), t("time"), t("collected"), t("deliveryFee"), t("rider"), t("status"), t("actions")].map(
                    (h) => (
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 flex-wrap gap-2">
            <span className="text-xs text-gray-500">
              {totalDocs > 0
                ? `${t("showing")} ${(serverPage - 1) * PAGE_SIZE + 1}–${Math.min(serverPage * PAGE_SIZE, totalDocs)} ${t("of")} ${totalDocs}`
                : `${t("showing")} ${filtered.length}`}
            </span>

            <div className="flex items-center gap-1">
              <button
                disabled={!canGoPrev || dispatchLoading}
              ...
              {pageWindow[0] > 1 && (
                <>
                  <button onClick={() => setPage(1)} className="w-7 h-7 rounded-lg text-xs hover:bg-gray-100 text-gray-600">1</button>
                  {pageWindow[0] > 2 && <span className="w-5 text-center text-gray-300 text-xs">…</span>}
                </>
              )}
              ...
              {pageWindow.map((p) => (
                <button
                  ...
                  className={`w-7 h-7 rounded-lg text-xs font-semibold disabled:opacity-50 ${
                    serverPage === p ? "bg-blue-600 text-white" : "hover:bg-gray-100 text-gray-600"
                  }`}
                >
                  {p}
                </button>
              ...
              {pageWindow[pageWindow.length - 1] < totalPages && (
                <>
                  {pageWindow[pageWindow.length - 1] < totalPages - 1 && <span className="w-5 text-center text-gray-300 text-xs">…</span>}
                  <button onClick={() => setPage(totalPages)} className="w-7 h-7 rounded-lg text-xs hover:bg-gray-100 text-gray-600">{totalPages}</button>
                </>
              )}

              <button
                disabled={!canGoNext || dispatchLoading}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {driversLoading && (
                    <tr>
                      <td colSpan={7} className="text-center py-8 text-gray-400 text-sm">{t("loadingDrivers")}</td>
                    </tr>
                  )}
                  {!driversLoading && drivers.length === 0 && (
                    <tr>
                      <td colSpan={7} className="text-center py-8 text-gray-400 text-sm">{t("noDrivers")}</td>
                    </tr>
                  )}
                  {drivers.map((driver) => {
                    const dId = driver.id ?? "";
              ...
              <div className="space-y-3">
                {recentActivity.slice(0, 6).map((item: any, i: number) => {
                  ...
                  const dotColor =
                    ...
                    status === "out-for-delivery" ? "bg-blue-500"   :
                    status === "failed"           ? "bg-red-500"    : "bg-orange-500";
                  const textColor =
                    status === "delivered"       ? "text-green-600" :
                    status === "out-for-delivery" ? "text-blue-600"  :
                    status === "failed"           ? "text-red-600"   : "text-orange-600";
