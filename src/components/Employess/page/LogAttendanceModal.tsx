...
import { useEmployees } from "../../Employess/services/Employeeservices";
...
const selectCls = (hasError: boolean) =>
  `w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 transition-all bg-white ${
    hasError ? "border-red-300 bg-red-50/50 focus:ring-red-200 text-red-800" : "border-slate-200 focus:ring-blue-500 text-slate-700"
  }`;
...
export default function LogAttendanceModal({ onClose, branchId }: Props) {
  ...
  // keep the full result so we can safely inspect multiple possible shapes
  const empResult = useEmployees(empParams);

  // normalize employees to an array regardless of the hook's return shape
  const employees: any[] = (() => {
    const r = empResult as any;
    // case: the hook returned an array directly
    if (Array.isArray(r)) return r;
    // case: { data: [...] }
    if (r && Array.isArray(r.data)) return r.data;
    // case: { data: { data: [...] } } (nested)
    if (r && r.data && Array.isArray(r.data.data)) return r.data.data;
    // case: { employees: [...] }
    if (r && Array.isArray(r.employees)) return r.employees;
    // fallback empty
    return [];
  })();
  ...
  const handleSubmit = async () => {
    const allTouched = Object.keys(form).reduce((acc, k) => ({ ...acc, [k]: true }), {} as Record<keyof FormFields, boolean>);
        ...
        <div className="border-t border-slate-100 mt-4 pt-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select name="employeeId" value={form.employeeId}
                onChange={handleChange} onBlur={() => handleBlur("employeeId")}
                className={selectCls(!!errors.employeeId)}>
                <option value="">Select Employee</option>
                {employees.map((emp: any) => {
                  const id = emp._id ?? emp.id ?? "";
                  return <option key={id} value={id}>{emp.fullName ?? emp.name ?? id}</option>;
                })}
              </select>
          ...
          <div>
            ...
            <select name="status" value={form.status}
              onChange={handleChange} onBlur={() => handleBlur("status")}
