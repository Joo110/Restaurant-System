    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-5 font-sans">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">{t("manageAndTrackAllYourSupplyChainPartners")}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
        {[
          { label: t("totalSuppliers"), value: suppliers.length, color: "text-slate-800" },
          { label: t("active"), value: suppliers.filter((s) => s.status === "Active").length, color: "text-green-600" },
          { label: t("inactive"), value: suppliers.filter((s) => s.status === "Inactive").length, color: "text-red-500" },
          { label: t("totalItemsSupplied"), value: suppliers.reduce((a, s) => a + s.itemsSupplied, 0), color: "text-blue-600" },
        ].map((stat) => (
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 sm:max-w-xs">
          ...
          <input
            type="text"
            placeholder={t("searchSupplierOrContact")}
            value={search}
          ...
          <table className="w-full text-sm min-w-[700px]">
            <thead className="border-b border-slate-100 bg-slate-50">
              <tr className="text-slate-500 font-semibold text-xs uppercase tracking-wide">
                <th className="py-3 px-4 sm:px-5 text-left">{t("supplier")}</th>
                <th className="py-3 px-4 sm:px-5 text-left">{t("contact")}</th>
                <th className="py-3 px-4 sm:px-5 text-left">{t("categories")}</th>
                <th className="py-3 px-4 sm:px-5 text-left">{t("bank")}</th>
                <th className="py-3 px-4 sm:px-5 text-left">{t("items")}</th>
                <th className="py-3 px-4 sm:px-5 text-left">{t("status")}</th>
                <th className="py-3 px-4 sm:px-5 text-left">{t("actions")}</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((s) => (
