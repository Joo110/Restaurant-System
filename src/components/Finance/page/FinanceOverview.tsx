...
import {
  ...
  periodToRange,
  type Period,
} from "../../dashboard/hook/useAccounts";
...
const Spark: React.FC<{ color: string }> = ({ color }) => {
  const d = useMemo(
    () => Array.from({ length: 12 }, () => ({ v: Math.random() * 100 })),
    []
  );
  return (
...
function filterMonthly(
  ...
  if (period === "thisWeek")
    return monthly.filter(
      (m) => m.month >= Math.max(1, cur - 2) && m.month <= cur
    );
  return monthly;
}
...
export default function FinanceOverview() {
  ...
  const [chartMode, setChartMode] = useState<"monthlyRevenue" | "profitAnalysis">(
    "monthlyRevenue"
  );
  ...
  const rawMonthly: { month: number; revenue: number }[] =
    finance?.monthlyRevenue ?? [];
  const filteredMonthly = useMemo(
    () => filterMonthly(rawMonthly, period),
    [rawMonthly, period]
  );
  ...
  const periodLabel =
    period === "today" ? t("today") : period === "thisWeek" ? t("thisWeek") : t("thisMonth");

  const statCards = [
    {
      label: `${t("totalRevenue")} — ${periodLabel}`,
      value: `$${totalRevenue.toLocaleString()}`,
      delta: finance?.summary?.totalRevenueChange ?? "0%",
      up: !String(finance?.summary?.totalRevenueChange ?? "").startsWith("-"),
      color: !String(finance?.summary?.totalRevenueChange ?? "").startsWith("-")
        ? "text-green-500"
        : "text-red-500",
      miniColor: "#22c55e",
    },
  ...
  return (
    <>
      ...
      <div className="screen-only space-y-4 sm:space-y-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {(["today", "thisWeek", "thisMonth"] as Period[]).map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`px-3 sm:px-4 py-1.5 rounded-xl text-xs sm:text-sm font-medium transition-all ${
                  period === p
            ...
            <select
              ...
              className="px-3 py-1.5 rounded-xl text-xs border border-slate-200 text-slate-600 bg-white outline-none"
            >
            ...
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl bg-blue-600 text-white text-xs sm:text-sm font-medium hover:bg-blue-700 transition-colors"
            >
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
          {statCards.map((card) => (
            <div
              key={card.label}
              className="bg-white rounded-2xl border border-slate-100 p-3 sm:p-4 shadow-sm"
            >
              ...
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                ...
                {card.delta && (
                  <span
                    className={`text-[10px] sm:text-xs font-semibold ${card.color}`}
                  >
                    {card.up ? "↑" : "↓"} {card.delta}
                  </span>
                )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm">
            ...
            <p className="text-[11px] sm:text-xs text-slate-400 mt-0.5 mb-4">
              {t("operationalCostDistribution")}
            </p>
                ...
                <table className="w-full min-w-[260px]">
                  ...
                  <tbody>
                    {expenseBreakdown.map((row: any) => (
                      ...
                      >
                        <td className="py-2 sm:py-2.5 text-xs sm:text-sm text-slate-700">
                          {row.category}
                        </td>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm">
            <h2 className="font-bold text-slate-900 text-sm sm:text-base">
              {t("revenueByOrderType")}
            </h2>
            <p className="text-[11px] sm:text-xs text-slate-400 mt-0.5 mb-2">
              {t("channelPerformanceDistribution")}
            </p>
      ...
      <div className="print-only bg-white text-slate-900 p-6 sm:p-8">
        <div className="print-section grid grid-cols-2 gap-3 mb-5">
          {statCards.map((card) => (
            <div key={card.label} className="border border-slate-200 rounded-xl p-3">
        ...
        <div className="print-section border border-slate-200 rounded-xl p-4 mb-5">
          <h2 className="font-bold text-slate-900 text-sm mb-3">
            {t("revenueAndProfitTrends")}
          </h2>
          {chartData.length === 0 ? (
            <p className="text-xs text-slate-500">{t("noRevenueDataForThisYear")}</p>
          ) : (
        ...
        <div className="print-section grid grid-cols-2 gap-5 mb-5">
          <div className="border border-slate-200 rounded-xl p-4">
            ...
            {expenseBreakdown.length === 0 ? (
              <p className="text-xs text-slate-500">{t("noExpenseData")}</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-slate-500">
          ...
          <div className="border border-slate-200 rounded-xl p-4">
            ...
            {orderTypeData.length === 0 ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {orderTypeData.map((row: any) => (
                    <tr key={row.name} className="border-b border-slate-100 last:border-0">
                      <td className="py-2 text-slate-700 capitalize">{row.name}</td>
                      <td className="py-2 text-right font-medium">
                        ${Number(row.amount ?? 0).toLocaleString()}
                      </td>
                    </tr>
        ...
        <div className="print-section border border-slate-200 rounded-xl p-4">
          <h2 className="font-bold text-slate-900 text-sm mb-3">
            {t("payrollSummary")}
          </h2>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg border border-slate-200 p-3">
              <p className="text-[11px] text-slate-500">{t("totalPayroll")}</p>
              <p className="text-lg font-bold mt-1">
                ${payrollCurrentYear.toLocaleString()}
              </p>
            </div>
            <div className="rounded-lg border border-slate-200 p-3">
              <p className="text-[11px] text-slate-500">{t("nextPayroll")}</p>
              <p className="text-lg font-bold mt-1">
                ${payrollNext.toLocaleString()}
              </p>
            </div>
