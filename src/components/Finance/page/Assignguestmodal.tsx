...
const ASSIGN_MODES = ["Assign Now", "Reserve"] as const;
type AssignMode = typeof ASSIGN_MODES[number];

export default function AssignGuestModal({
  isOpen, tableId, realId, onClose, onConfirm,
}: AssignGuestModalProps) {
  const [mode,          setMode]          = useState<AssignMode>("Assign Now");
  ...
  const validate = (): FormErrors => {
    ...
    if (!guestName.trim())
      ...
      e.guestName = "Name must be at least 2 characters.";

    if (customerPhone && !/^[+\d\s\-()]{7,20}$/.test(customerPhone))
      e.customerPhone = "Enter a valid phone number.";

    if (mode === "Reserve" && !reservedFor)
      e.reservedFor = "Please select a reservation date and time.";

    return e;
  };

  const reset = () => {
    setGuestName(""); setCustomerPhone(""); setGuests(1);
    setWaiterId(""); setReservedFor(""); setErrors({}); setMode("Assign Now");
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-blue-50 rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-xl font-bold text-slate-900">
          {mode === "Reserve" ? "Reserve Table" : "Assign Guest"}
        </h2>
        <p className="text-sm text-slate-500 mt-1 mb-4">
          {mode === "Reserve" ? "Reserve" : "Assign a guest to"} {tableId}
        </p>
          ...
          {ASSIGN_MODES.map((m) => (
            <button key={m} onClick={() => { setMode(m); setErrors({}); }}
              className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all ${
                mode === m ? "bg-blue-600 text-white" : "border border-slate-200 text-slate-600 hover:bg-slate-50"
              }`}
            >{m}</button>
          ))}
        </div>
        ...
        <div className="mb-4">
          ...
          <input type="text" placeholder="Enter guest name"
            value={guestName}
            onChange={(e) => { setGuestName(e.target.value); if (errors.guestName) setErrors((p) => ({ ...p, guestName: undefined })); }}
        ...
        <div className="mb-4">
          ...
          <input type="tel" placeholder="+20 1XX XXX XXXX"
            value={customerPhone}
            onChange={(e) => { setCustomerPhone(e.target.value); if (errors.customerPhone) setErrors((p) => ({ ...p, customerPhone: undefined })); }}
        ...
        {/* Reserved For — only in Reserve mode */}
        {mode === "Reserve" && (
          <div className="mb-4">
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
              Reserved For <span className="text-red-400">*</span>
            </label>
            <input type="datetime-local"
              value={reservedFor}
              onChange={(e) => { setReservedFor(e.target.value); if (errors.reservedFor) setErrors((p) => ({ ...p, reservedFor: undefined })); }}
        ...
        {/* Assign Waiter — only in Assign Now mode */}
        {mode === "Assign Now" && (
          <div className="mb-6">
        ...
        <div className="flex justify-end gap-3 mt-2">
          <button onClick={() => { reset(); onClose(); }} disabled={isLoading}
          ...
          <button onClick={handleConfirm} disabled={isLoading}
            ...
            {isLoading ? "Saving..." : mode === "Reserve" ? "Confirm Reservation" : "Confirm Assignment"}
          </button>
        </div>
      </div>
    </div>
