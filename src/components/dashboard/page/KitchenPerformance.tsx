...
const KitchenPerformance: React.FC<KitchenPerformanceProps> = ({ performance }) => {
  ...
  const meals = [
    { key: "breakfast" as const, label: t("dashboard.kitchenPerformance.breakfast"), color: "#F97316" },
    { key: "lunch" as const,     label: t("dashboard.kitchenPerformance.lunch"),     color: "#3B82F6" },
    { key: "dinner" as const,    label: t("dashboard.kitchenPerformance.dinner"),    color: "#8B5CF6" },
  ];

  return (
    <div className="bg-white rounded-2xl p-4 sm:p-5 border border-gray-100 shadow-sm">
      <h2 className="text-sm font-semibold text-gray-700 mb-1">
        {t("dashboard.kitchenPerformance.title")}
      </h2>
      <p className="text-xs text-gray-400 mb-4 sm:mb-5 flex items-center gap-1.5">
        <Clock size={11} />
        {t("dashboard.kitchenPerformance.subtitle")}
      </p>
          ...
          return (
            <div key={m.key} className="flex items-center gap-3 sm:gap-4">
              ...
              <span className="w-14 text-right text-xs sm:text-sm text-gray-400 flex-shrink-0">
                {val === "N/A" ? "N/A" : `${val} ${t("dashboard.kitchenPerformance.min")}`}
              </span>
            </div>
          );
        })}
