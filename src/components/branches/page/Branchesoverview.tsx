// src/pages/BranchesOverview.tsx
...
const BranchCard = ({
  ...
  >
    ...
    <div className="grid grid-cols-2 gap-4 mb-4">
      <div>
        <p className="text-[10px] font-semibold text-blue-400 uppercase tracking-wider mb-1">Daily Revenue</p>
      ...
      <div>
        <p className="text-[10px] font-semibold text-blue-400 uppercase tracking-wider mb-1">Total Orders</p>
    ...
    <div className="border-t border-gray-50 pt-3">
      <div className="flex justify-between items-center mb-1.5">
        <p className="text-[10px] font-bold text-blue-400 uppercase tracking-wider">Occupancy</p>
...
export default function BranchesOverview() {
  ...
  const mapToBranch = (b: any, idx: number): BranchUI => {
    // name
    const name = b?.name ?? b?.title ?? `Branch ${idx + 1}`;
  ...
  const handleDelete = async (id?: string, name?: string) => {
    ...
    const confirmMsg = `Are you sure you want to delete "${name ?? id}"? This action cannot be undone.`;
    ...
    try {
      ...
      alert("Failed to delete branch. See console for details.");
      // eslint-disable-next-line no-console
  ...
  return (
    <div className="space-y-6">
      {/* Page title */}
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Branches Overview</h2>
        <p className="text-sm text-gray-400 mt-0.5">Manage and monitor all your restaurant locations.</p>
      </div>
