...
export default function ConfirmReorderModal({ item, onClose, onSuccess }: Props) {
  ...
  const fc = (err?: string) =>
    `w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 transition-colors ${
      err ? "border-red-400 focus:ring-red-400 bg-red-50"
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="border-t border-slate-100 pt-5 space-y-4">
            ...
            <div>
              ...
              {unitPrice > 0 && (
                <p className="text-xs text-slate-400 mt-1">Unit price: ${unitPrice} / {unit}</p>
              )}
            </div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between text-slate-500">
                <span className="truncate pr-2">Subtotal ({qty} {unit} × ${unitPrice})</span>
