...
export default function AddEmployeeModal({ onClose, branchId }: Props) {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setGeneralError("Image must be under 2MB.");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await createEmployeeFn(payload as any);
              ...
              <div className="relative">
                <span
                  className={`absolute left-3 top-1/2 -translate-y-1/2 text-sm font-medium ${
                    errors.startingSalary ? "text-red-400" : "text-slate-400"
                  }`}
                >
                  $
                </span>
                <input
                  ...
                  onChange={handleChange}
                  onBlur={() => handleBlur("startingSalary")}
