...
function validate(form: FormFields): Errors {
  ...
  if (!form.startingSalary || isNaN(Number(form.startingSalary)))
    e.startingSalary = "Enter a valid salary amount.";
  else if (Number(form.startingSalary) < 0)
    e.startingSalary = "Salary cannot be negative.";
...
export default function AddEmployeeModal({ onClose, branchId }: Props) {
  // branchId is passed directly from StaffPage (which reads it from outlet context)
  // This is the same pattern as AddMenuItemModal + MenuPage
  ...
  const [form, setForm] = useState<FormFields>({
    ...
    startingSalary: "15000", bankName: "", accountNumber: "",
  });
  ...
  const handleSubmit = async () => {
    ...
    if (!branchId) {
      setGeneralError("Please select a branch before adding an employee.");
      return;
    }
    ...
    try {
      await createEmployeeFn({
        ...
        bankAccount:    form.bankName || form.accountNumber
      ...
      // merge API field errors into state so they show under the right inputs
      if (Object.keys(fieldErrors).length > 0) {
        setErrors((prev) => ({ ...prev, ...fieldErrors }));
        // also mark those fields as touched so errors are visible
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-5 sm:p-6 font-sans max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-lg sm:text-xl font-bold text-slate-900">Add New Employee</h2>
        ...
        <div className="border-t border-slate-100 pt-5 space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {previewUrl
                ...
                : <><span className="text-xl">📷</span><span className="text-[9px] text-blue-500 font-medium mt-0.5">Upload</span></>
              }
            </div>
            <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-slate-700">Profile Photo</p>
              <p className="text-xs text-slate-400 mb-2">Square image, max 2MB.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">
                Starting Salary <span className="text-red-400">*</span>
              </label>
              <div className="relative">
                <span className={`absolute left-3 top-1/2 -translate-y-1/2 text-sm font-medium ${errors.startingSalary ? "text-red-400" : "text-slate-400"}`}>$</span>
                <input name="startingSalary" type="number" placeholder="0"
