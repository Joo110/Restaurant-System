    ...
    if (Array.isArray(list) && list.length > 0) {
      ...
      list.forEach((e: { path?: string; msg?: string }) => {
        if (e.path && e.msg) {
          ...
          if (key && !fieldErrors[key]) fieldErrors[key] = e.msg;
          else if (!key && !generalError) generalError = e.msg ?? null;
        }
      });
...
function validate(form: FormFields): Errors {
  ...
  if (!form.currentSalary || isNaN(Number(form.currentSalary))) e.currentSalary = "Enter a valid salary amount.";
  else if (Number(form.currentSalary) < 0)               e.currentSalary = "Salary cannot be negative.";
...
export default function EditEmployeeModal() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      currentSalary: emp.currentSalary
                       ...
                       : emp.startingSalary ? String(emp.startingSalary) : "",
  ...
  if (isError || !emp) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center font-sans">
        <p className="text-slate-400 text-sm">{t("employeeNotFound")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-start sm:items-center justify-center p-4 font-sans">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-5 sm:p-6 max-h-[95vh] overflow-y-auto">
        ...
        <h2 className="text-lg sm:text-xl font-bold text-slate-900">{t("editEmployeeProfile")}</h2>
        <p className="text-sm text-slate-400 mt-0.5 mb-5">
          {t("modifyDetails")}{" "}
          <span className="font-medium text-slate-600">{emp.fullName}</span>
        </p>
        ...
        {success && (
          <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3 mb-4">
            ...
            <div>
              <p className="text-sm font-semibold text-green-700">{t("profileUpdated")}</p>
              <p className="text-xs text-green-500 mt-0.5">{t("redirectingBack")}</p>
            </div>
          </div>
        )}
        ...
        <div className="border-t border-slate-100 pt-5 space-y-4">
          ...
          <div className="flex flex-col items-center gap-3 mb-2">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-slate-400">{t("clickToChangePhoto")}</p>
            </div>
          </div>
          ...
          {/* Start Date */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">{t("startDate")}</label>
            <input
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">
                {t("currentSalary")} <span className="text-red-400">*</span>
              </label>
              <div className="relative">
                ...
                <input
                  name="currentSalary" type="number" placeholder="0"
        ...
        <div className="flex gap-3 mt-6 justify-end">
          ...
          >
            ...
            {isSubmitting ? t("saving") : success ? t("saved") : t("updateProfile")}
          </button>
        </div>
      </div>
    </div>
