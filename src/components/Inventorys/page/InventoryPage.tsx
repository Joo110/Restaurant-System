...
export default function InventoryPage() {
  ...
  const STATUS_FILTERS = [t("totalItems"), t("criticalLow"), t("reorderSoon")];

  const filtered = items.filter((item) => {
    const pct = getStockPercent(item.currentQuantity ?? 0, item.targetQuantity ?? 1);
    if (activeFilter === t("criticalLow")) return pct <= 20;
    if (activeFilter === t("reorderSoon")) return pct > 20 && pct <= 50;
    return true;
  });
